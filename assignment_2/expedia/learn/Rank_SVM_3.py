...
features_engineered = ['log10_price_usd', 'starrating_diff', 'usd_diff', 'prob_book_prop_id', 'prob_click_prop_id','prop_starrating_monotonic', 'price_usd_normalized_search_id','price_usd_normalized_prop_id','price_usd_normalized_srch_destination_id']
...
train_set = train_set.replace(float('inf'),0,regex=True)
train_set= train_set.replace(float('NaN'),0,regex=True)

#Click training
y = train_set['click_bool']
X = train_set[numerical_attributes + features_engineered]
# clf_click = svm.SVC(class_weight = 'auto')
clf_click = ensemble.GradientBoostingClassifier()
clf_click.fit(X,y)
...
clf_book = ensemble.GradientBoostingClassifier()
clf_book.fit(X,y)

print('done with training on bookings')

###Do the same feature engineering on test_set########
###############################################################################################################################################
...
test_set = test_set.replace(float('inf'),0,regex=True)
test_set= test_set.replace(float('NaN'),0,regex=True)
