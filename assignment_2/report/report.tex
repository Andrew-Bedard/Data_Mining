...
If we add single numerical attributes without aggregate variables we're actually able to get close to the score without adding anything, but not outperform it by any significant margin. We look at another trainingset with negative instance being undersampled that has a baseline score of \textbf{0.448} (the score partially vary because of the undersampling, which has a bigger influence with the smaller subset than when we're training on the full trainingset).Adding \verb!prop_log_historical_price!, \verb!srch_saturday_night_bool! or \verb!prop_starrating! gives scores of respectively \textbf{0.442}, \textbf{0.442} and \textbf{0.447}.\\
...
\section{Discussion}
regression model\\
more systematic checking of small subsets of numerical attributes\\
actual ensemble
\section{Conclusions}
